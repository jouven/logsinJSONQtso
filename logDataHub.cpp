...
std::unordered_set<messageType_ec> logDataHub_c::logTypesToSave_f() const
{
    return logTypesToSave_pri;
}

void logDataHub_c::setLogTypesToSave_f(const std::unordered_set<messageType_ec>& logTypesToSave_par_con)
{
...
bool logDataHub_c::partialClearWhenMaxMessages_f() const
{
    return partialClearWhenMaxMessages_pri;
}

void logDataHub_c::setPartialClearWhenMaxMessages_f(const bool partialClearWhenMaxMessages_par_con)
{
    partialClearWhenMaxMessages_pri = partialClearWhenMaxMessages_par_con;
}

bool logDataHub_c::partialClearWhenMaxUniqueMessages_f() const
{
    return partialClearWhenMaxUniqueMessages_pri;
}

void logDataHub_c::setClearWhenMaxUniqueMessages_f(const bool partialClearWhenMaxUniqueMessages_par_con)
{
    partialClearWhenMaxUniqueMessages_pri = partialClearWhenMaxUniqueMessages_par_con;
}

bool logDataHub_c::rotateWhenMaxLogFileSize_f() const
{
    return rotateWhenMaxLogFileSize_pri;
}

void logDataHub_c::setRotateWhenMaxLogFileSize_f(const bool rotateWhenMaxLogFileSize_par_con)
{
    rotateWhenMaxLogFileSize_pri = rotateWhenMaxLogFileSize_par_con;
}

std::unordered_set<messageType_ec> logDataHub_c::logTypesToSaveFile_f() const
{
    return logTypesToSaveFile_pri;
}

void logDataHub_c::setLogTypesToSaveFile_f(const std::unordered_set<messageType_ec>& logTypesToSaveFile_par_con)
{
        ...
        , const QString& threadId_par_con)
{
    ...
    while (true)
    {
        ...
        if (
            (partialClearWhenMaxMessages_pri and maxMessagesReachedTmp)
            or (partialClearWhenMaxUniqueMessages_pri and maxUniqueMessagesReachedTmp)
            )
        {
            partialMemoryLogClear_f();
        }
        else
        {
        ...
        const QString& typeStrTmp(messageTypeToString_f(type_par_con));
...
//        }
        ...
        if (saveLogFiles_pri and isValidLogPathBaseName_pri and logTypesToSaveFile_pri.count(type_par_con) == 1)
        {
            ...
            if (logFile_pri.size() >= maxLogFileByteSize_pri)
            {
                ...
                logFile_pri.setFileName(currentLogFileName_pri);
                if (not fileSaveLogTypeText_pri)
                {
                    currentIndexToSaveFrom_pri = indexTmp_con;
                }
            }
...
bool logDataHub_c::loadLogFiles_f(
        ...
        , const bool keepUsingLoadedLogFilePath_par_con
        , const bool errorWhenNoneFound_par_con)
{
    bool resultTmp(false);
    ...
    while (true)
    {
        ...
        if (logFilePath_par_con.isEmpty())
        {
        ...
        {
            pathNameTmp = logFilePath_par_con;
        }
#ifdef DEBUGJOUVEN
        ...
        if (defaultCaseTmp)
        {
            ...
            entriesTmp = parentTmp.entryList({executableName_f() + "*_UTC*.log"}, QDir::Files, QDir::Name);
        }
        else
        {
            QDir parentTmp(fileInfoTmp.path());
            entriesTmp = parentTmp.entryList({fileInfoTmp.fileName() + "*_UTC*.log"}, QDir::Files, QDir::Name);
        }

        if (entriesTmp.isEmpty())
        {
            if (errorWhenNoneFound_par_con)
            {
                appendError_f({"No log files found with file base name: {0}", pathNameTmp});
            }
            break;
        }
...
//#endif
        ...
        {
            readLogResultTmp = readLogFile_f(entriesTmp.last(), logFilter_par_con);
        }
        else
        {
            for (const QString& filename_ite_con : qAsConst(entriesTmp))
            {
                readLogResultTmp = readLogFile_f(filename_ite_con, logFilter_par_con);
            }
        }

        //keep logging in the last loaded log, use its directory too
        if (keepUsingLoadedLogFilePath_par_con and readLogResultTmp)
        {
//#ifdef DEBUGJOUVEN
//            qtOutRef_ext() << "entriesTmp.first() " << entriesTmp.first() << endl;
//#endif
            currentLogFileName_pri = entriesTmp.last();
...
//bool logDataHub_c::changeLogFilePath_f(
//        const QString& logFilePath_par_con
//        , const bool createParentDirectories_par_con)
//{
//    bool resultTmp(false);
//    while (true)
//    {
//        QFileInfo fileInfoTmp(logFilePath_par_con);
//        if (fileInfoTmp.exists())
//        {
//            appendError_f({"Log file change, failed a file already exists: {0}", logFilePath_par_con});
//            break;
//        }

//        QDir parentTmp(fileInfoTmp.dir());
//        if (not parentTmp.exists())
//        {
//            if (createParentDirectories_par_con)
//            {
//                if (parentTmp.mkpath("."))
//                {
//                }
//                else
//                {
//                    appendError_f({"Log file change, couldn't create parent directory: {0}", parentTmp.path()});
//                    break;
//                }
//            }
//        }
//        else
//        {
//            appendError_f({"Log file change, parent directory path doesn't exist: {0}", parentTmp.path()});
//            break;
//        }

//        QMutexLocker lockerTmp(std::addressof(addMessageMutex_pri));

//        currentLogFileName_pri
//        logSaveDirectoryPath_pri
...
QString logDataHub_c::getNextFilePath_f()
{
    ...
    resultTmp = logSaveDirectoryPath_pri + "/" + logBaseFilename_pri + "_" + currentDateTimeValue_pri + "_UTC.log";
    if (QFileInfo::exists(resultTmp))
    {
        int_fast32_t indexTmp(0);
        QString commonStringTmp(logSaveDirectoryPath_pri + "/" + logBaseFilename_pri + "_" + currentDateTimeValue_pri + "_UTC_");
        do
        {
            indexTmp = indexTmp + 1;
...
)
{
    ...
    while (true)
    {
        ...
        if (fileSaveLogTypeText_pri)
        {
            ...
            {
                //qtOutLine_f("line size " + QString::number(lineTmp_ite_con.size()));
                //20191114 read log lines, ignore the ones that aren't read successfully
                if (lineTmp_ite_con.size() > 0)
                {
                    //implicit QByteArray to QString ctor conversion
                    readTextLine_f(lineTmp_ite_con, logFilter_par_con);
                }
//                else
//                {
//                    break;
...
void logDataHub_c::setLogSaveDirectoryPath_f(
        ...
        , const QString& logBaseFilename_par_con
        , const bool keepOldLogPathOnError_par_con
)
{
    bool errorTmp(false);
    bool isValidOldTmp(isValidLogPathBaseName_pri);
    QString logSaveDirectoryPathOldTmp(logSaveDirectoryPath_pri);
    QString logBaseFilenameOldTmp(logBaseFilename_pri);
    while (true)
    {
        QString logSaveDirectoryPathTmp;
        QString logBaseFilenamePathTmp;
        ...
        if (logBaseFilename_par_con.isNull())
        {
            logBaseFilenamePathTmp = executableName_f();
        }
        else
        {
            logBaseFilenamePathTmp = logBaseFilename_par_con;
        }
        ...
        if (not logSaveDirectoryPathDirTmp.exists())
        {
            errorTmp = true;
            //append/print error
            text_c errorMsgTmp("Directory {0} doesn't exist", logSaveDirectoryPathTmp);
            appendError_f(errorMsgTmp);
            break;
        }
        else
        {
            ...
            logBaseFilename_pri = logBaseFilenamePathTmp;
        }

        //test creating a log file, this function checks if the new file already exists,
        //if it does it will modify the filename adding an increasing index until it founds one it doesn't exist
        ...
        else
        {
            errorTmp = true;
            //append/print error
            text_c errorMsgTmp("Could not save log entry, file {0} could not be opened", currentLogFileNameTmp);
            appendError_f(errorMsgTmp);
        }
        break;
    }
    if (errorTmp)
    {
        ...
        {
            logSaveDirectoryPath_pri = logSaveDirectoryPathOldTmp;
            logBaseFilename_pri = logBaseFilenameOldTmp;
            isValidLogPathBaseName_pri = isValidOldTmp;
        }
        else
        {
...
std::vector<bool> logDataHub_c::addMessage_f(
        ...
        , const int_fast32_t sourceLineNumber_par_con)
{
    ...
    resultTmp.reserve(message_par_con.size_f());
    for (size_t i = 0; i < message_par_con.size_f(); ++i)
    {
        resultTmp.emplace_back(addMessage_f(
                    message_par_con.text_f(i)
                    , reference_par_con
                    , type_par_con
                    , sourceFile_par_con
                    , sourceFunction_par_con
...
{
    QString resultTmp;
    resultTmp.append(datetime_par_con.toString("yyyy-MM-dd hh:mm:ss.zzz")).append('\t'); //1
    resultTmp.append(logItem_par->threadId_f()).append('\t');// 2
    resultTmp.append(messageTypeToString_f(logItem_par->type_f())).append('\t');// 3
...
//        }
    ...
    resultTmp.append(messageTmp).append('\t');//4
    if (translatedTmp)
    {
        resultTmp.append("T\t");//5
    }
    else
    {
        resultTmp.append("U\t");//5
    }
    QString referenceTmp(logItem_par->reference_f());
    referenceTmp.remove('\n').remove('\t');
    resultTmp.append(referenceTmp).append('\t');//6
    resultTmp.append(logItem_par->sourceFileName_f()).append('\t');//7
    resultTmp.append(logItem_par->sourceFunctionName_f()).append('\t');//8
    resultTmp.append(QString::number(logItem_par->sourceLineNumber_f())).append('\t');//9
...
)
{
    ...
    while (true)
    {
        ...
        {
            appendError_f({"Only {0} fields, less than 11 per log line, tab number is {1}", fieldsTmp.size(), line_par.count('\t')});
            break;
        }
        ...
        messageType_ec typeTmp(stringToMessageTypeTo_f(typeStrTmp));
        if (typeTmp == messageType_ec::empty)
        {
            appendError_f("Unknown message type");
            break;
        }
        ...
        {
            appendError_f("Empty message log line");
            break;
        }
...
void logDataHub_c::write_f(QJsonObject& json_par) const
{
    ...
    QMap<int_fast64_t, std::pair<logItem_c*, QDateTime>>::ConstIterator startingIteratorTmp(currentIndexToSaveFrom_pri > 0 ? indexToLogItemPtrAndDatetimeMap_pri.find(currentIndexToSaveFrom_pri) : indexToLogItemPtrAndDatetimeMap_pri.begin());
...
//            }
        ...
        jsonObjectTmp["type"] = messageTypeToString_f(logItemPtrTmp->type_f());
...
void logDataHub_c::partialMemoryLogClear_f()
{
    QMutexLocker lockerTmp(&addMessageMutex_pri);
    //copy half of the logs in temporal containers
    auto midIndex(indexToLogItemPtrAndDatetimeMap_pri.begin() + (indexToLogItemPtrAndDatetimeMap_pri.size() / 2));
    QMap<int_fast64_t, std::pair<logItem_c*, QDateTime>> tmpMap;
    std::unordered_map<logItem_c*, logItem_c> tmpVector;
    tmpVector.reserve(indexToLogItemPtrAndDatetimeMap_pri.size() / 2);
    while (midIndex not_eq indexToLogItemPtrAndDatetimeMap_pri.end())
    {
        tmpMap.insert(midIndex.key(), midIndex.value());
        tmpVector.emplace(midIndex.value().first, *midIndex.value().first);
        ++midIndex;
    }

    //clear the log containers
    indexToLogItemPtrAndDatetimeMap_pri.clear();
    messageSizeUMap_hashElementUMap_pri.clear();

    //restore values
    indexToLogItemPtrAndDatetimeMap_pri.swap(tmpMap);
    for (const auto& logItemPair_ite_con : tmpVector)
    {
        const QString& typeStrTmp(messageTypeToString_f(logItemPair_ite_con.first->type_f()));

        QString messageToHashTmp(
                    logItemPair_ite_con.first->message_f().rawReplace_f()
                    + logItemPair_ite_con.first->reference_f()
                    + typeStrTmp
                    + logItemPair_ite_con.first->sourceFileName_f()
                    + logItemPair_ite_con.first->sourceFunctionName_f()
                    + QString::number(logItemPair_ite_con.first->sourceLineNumber_f())
        );
        //minor warning, with the addition of text_c, a log message could be inserted already translated or not,
        //this means "memory saving" won't be as effective as with simple string and no translation
        //although in theory inserted message aren't translated (actonQtso and actonQtg do this),
        ...
        if (findSizeResultTmp not_eq messageSizeUMap_hashElementUMap_pri.end())
        {
            ...
            {
                findSizeResultTmp->second.emplace(hashResultTmp, logItemPair_ite_con.second);
            }
        }
        else
        {
            std::unordered_map<uint_fast64_t, logItem_c> hashElementUMapTmp;
            hashElementUMapTmp.emplace(hashResultTmp, logItemPair_ite_con.second);

            messageSizeUMap_hashElementUMap_pri.emplace(messageSizeTmp, hashElementUMapTmp);
        }
    }

    //if log file is in json rotate, because unlike "text type" json saves-replaces everything in the containers, in memory, to the current log file
    //and saving after clear would remove the old log entries
    if (saveLogFiles_pri and not fileSaveLogTypeText_pri)
    {
        currentIndexToSaveFrom_pri = atomicLogIndex_pri + 1;
...
std::vector<messageType_ec> logFilter_c::types_f() const
{
    return types_pri;
}

void logFilter_c::setTypes_f(const std::vector<messageType_ec>& types_par_con)
{
